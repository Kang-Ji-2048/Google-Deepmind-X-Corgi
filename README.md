...
**Immediate photo demo:** click **Choose photo** and attach any supported picture, or take a camera photo. A fixed BBC Good Food chicken-and-mushroom hotpot card and **View recipe** link appear immediately, with no second search click. Normal product copy is used with a small **Demo** marker and a brief note that ingredient matching is not connected. No image upload, model call, fake ingredient extraction, API key or quota is needed. The card remains while at least one photo is selected and disappears when all are removed. It is not personalized or checked against dietary requirements.

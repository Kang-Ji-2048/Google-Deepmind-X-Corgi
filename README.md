...
**Photo preview flow:** click **Choose photo** and attach any supported picture, or take a camera photo. Choose a **Cuisine type** (including **British**, plus Any cuisine, Italian, Mexican, Indian, Japanese, Chinese, Thai and Mediterranean) and **Cooking time** (Any time, 15, 30, 45 or 60 minutes), then press **Done**. Only then does the fixed Good Food Middle East **Roasted vegetables** card and original publisher **View recipe** link appear. **Change preferences** returns to the controls with selections retained; adding or removing a photo also requires Done again. Per the requested visual preview, demo labels and unavailable-matching messages are omitted from the card. No image upload, model call, fake ingredient extraction, API key or quota is needed. This remains a hardcoded visual prototype: cuisine/time selections are local UI state only and do not filter the fixed recipe or verify dietary requirements. The Gemma owner must connect confirmed ingredients and these preferences to real recipe search for the final flow.
...
Current verification: 33 automated tests, production build and typecheck pass, including the new preference/Done transitions; a live TheMealDB smoke request returned source-backed recipes. The prior public Vercel deployment passed homepage and recipe-route smoke checks. Real phone cameras and the external Gemma service are still unverified.
